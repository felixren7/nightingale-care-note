...
story += [Paragraph("72-hour scope", styles["H2N"]), Paragraph("Implemented: role-owned entries, longitudinal timeline, threaded coordination, tasks, full revisions and word diffs, append-only revert, deterministic 409 conflicts, patient-safe instructions and updates, three scribe interaction types, exact provenance, reversible importance feedback, connection-aware SSE, admin audit, and verified cold archive. Deliberately excluded: real LLM, voice capture, transcription, diarization, multilingual audio, and EHR integration.", styles["BodyN"])]
...
story += [Paragraph("Patient safety is a server response property: patient DTOs contain only visibility=patient instructions and patient-authored insights; highlights, tasks, comments, raw AI notes, artifacts, and audit fields do not exist in the payload.", styles["Callout"]), PageBreak()]
...
story += [Paragraph("Collaboration and realtime", styles["H2N"]), Paragraph("Encrypted threaded comments support assignment and resolve/reopen; History and Comments are filtered views. Tasks remain separate actionable records. Each mutation writes metadata-only audit plus a patient-scoped outbox event in the same transaction. A reconnectable 25-second SSE stream queries only after patient authorization and exposes connecting/live/reconnecting state; another browser refreshes without receiving unauthorized payload fields.", styles["BodyN"])]
...
story += [Paragraph("Self-learning importance", styles["H1N"]), Paragraph("Base score combines risk (critical +40, high +25, medium +10), unresolved task (+25), clinician confirmation (+15), entity (allergy +15, medication +10, chief complaint +8), and freshness (up to +15). Accept/pin adds +2, reject subtracts 2, and undo_reject restores +2 to a clinic-scoped featureKey weight, bounded to [−15,15]. Matching suggestions recompute finalScore = clamp(base + learned, 0, 100). Feedback, score, reason, and source remain auditable; rejection never removes provenance.", styles["BodyN"])]
...
story += [Paragraph("Measured acceptance", styles["H1N"]), metric_cards(), Spacer(1, 4 * mm), Paragraph("Vitest verifies encryption, redaction-before-provider, scoring, and archive round-trip. Five brief-named Python files make real HTTP calls and run ten cases covering cross-role writes, cross-clinic hiding, patient field omission and patient write persistence, revisions/revert/metadata-only audit, exact spans, separate-entry concurrency, deterministic same-entry 409, +2 learned score, and reject-undo recovery. A binary scan confirms known free-text phrases are absent from SQLite.", styles["BodyN"])]
tradeoffs = [
...
story += [Paragraph("Trade-offs and next boundary", styles["H1N"]), trade, Spacer(1, 4 * mm), Paragraph("Before real data: trusted TLS termination, managed identity, CSRF/rate limits, rotating secrets, formal threat modeling, retention policy, restore drills, formal accessibility and clinical safety evaluation, and observability without content. The prototype optimizes for trust boundaries and reproducibility—not compliance claims.", styles["Callout"])]
